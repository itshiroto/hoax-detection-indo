...
def fact_check(query: str, use_vector_db: bool = True, use_tavily: bool = True) -> Optional[FactCheckResponse]:
...
def main():
    ...
    parser.add_argument(
        "query",
        help="News title or content to check"
    )
    parser.add_argument(
        "--no-vector-db",
        action="store_true",
        help="Skip vector database search"
    )
    parser.add_argument(
        "--no-tavily",
        action="store_true",
        help="Skip Tavily web search"
    )
